...
fun View.cancelFade() {
    val tagKey = "fadeTo".hashCode()
    val visible = getTag(tagKey) as? Boolean
    animate().cancel()
    visible?.let {
        isVisible = visible
        alpha = if (visible) getTag("fadeToAlpha".hashCode())?.let { it as Float } ?: 1f else 0f
    }
...
fun View.cancelFadeRecursively() {
    ...
    viewGroup?.children?.asSequence()?.forEach { it.cancelFade() }
}
