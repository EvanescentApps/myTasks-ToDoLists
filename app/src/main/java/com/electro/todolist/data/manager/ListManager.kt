...
class ListManager(private val context: Context) {
    ...
    var currentListName: String // No longer lateinit, initialized in getDefaultList or by setCurrentListName

    // --- Initialization and Default List Logic ---

    init {
        // Initialize currentListName to a default value, will be set properly by getDefaultList
        currentListName = listsPrefs.getString(LAST_OPENED_LIST_KEY, "") ?: ""
     ...
     * and sets the `currentListName` to the last opened list or a default.
     */
    fun getDefaultList() {
        ...
        if (allLists.isEmpty()) {
            ...
            createDefaultLists()
            // After creating, get them again to ensure `allLists` is populated
            val newlyCreatedLists = getAllLists()
            currentListName = newlyCreatedLists.firstOrNull()?.id ?: "defaultList" // Set to first list's ID
        } else {
            // Lists exist, try to set currentListName to last opened or first available
            val lastOpenedId = listsPrefs.getString(LAST_OPENED_LIST_KEY, null)
            currentListName = if (lastOpenedId != null && allLists.any { it.id == lastOpenedId }) {
                lastOpenedId
            } else {
                allLists.firstOrNull()?.id ?: "defaultList" // Fallback to first list or a hardcoded default
            }
        }
        Timber.tag("ListManager").d("Default list set to: $currentListName")
        // Persist the current list ID
        listsPrefs.edit { putString(LAST_OPENED_LIST_KEY, currentListName) }
    }
